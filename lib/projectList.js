...
module.exports = class ProjectList {
  ...
  elementForItem(item, options) {
    ...
    let indices =
      this.query.length > 0
        ? atom.ui.fuzzyMatcher.match(item.text, this.query, {
  ...
  getLoadingMessage(mode) {
    return mode
      ? [
          <span>{`Indexing project \u2026 ${
            Number.isInteger(mode) ? mode : ""
          }`}</span>,
  ...
  scanDir(dirPath, tags, scanList) {
    return new Promise((resolve, reject) => {
      const taskPath = require.resolve("./projectScan");
      if (scanList == true) {
        scanList = "*/";
      }
