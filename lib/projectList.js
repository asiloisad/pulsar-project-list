    ...
    if (this.parseTitleTagsEnabled) {
      ...
      for (let part of titleParts) {
        if (part.isTag) {
          ...
          // Account for the '[' character in the offset
          total += 1
          this.highlightInElement(et, part.text, indices.map(x => x - total))
          // Account for the tag text and ']' character
          total += part.text.length + 1
          e1.appendChild(et)
        } else {
          this.highlightInElement(e1, part.text, indices.map(x => x - total))
