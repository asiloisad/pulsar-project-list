...
module.exports = class RecentList {
  constructor() {
    ...
    this.slv.element.classList.add("recent-list");
  ...
  getInfoMessage() {
    return this.showKeystrokes
      ? [
          ...
          ", ",
          <span class="keystroke">Shift-Enter</span>,
          " or ",
          <span class="keystroke">Alt-V</span>,
        ]
      : null;
  }
  ...
  filter(items, query) {
    ...
    for (const item of items) {
      ...
      for (let i = 0; i < item.texts.length; i++) {
        let score = atom.ui.fuzzyMatcher.score(item.texts[i], this.query);
        if (score > item.score) {
          item.score = score;
          item.ibest = i;
        }
  ...
  elementForItem(item, options) {
    ...
    for (let i = 0; i < item.paths.length; i++) {
      ...
      if (i === item.ibest) {
        let indices =
          this.query.length > 0
            ? atom.ui.fuzzyMatcher.match(item.texts[item.ibest], this.query, {
    ...
    li.addEventListener("contextmenu", () => {
      this.slv.selectIndex(options.index);
    });
    return li;
  }
