...
module.exports = class RecentList {
    ...
    return new Promise((resolve) => {
      for (let project of atom.history.getProjects()) {
        this.items.push({
          ...
          texts: project.paths.map((ppath) => { return Diacritics.clean(ppath.split(/[\\\/]/g).join(path.sep) + path.sep) }),
        })
      }
      resolve()
    })
  ...
  elementForItem(item, options) {
    ...
    for (let i = 0; i < item.paths.length; i++) {
      ...
      if (i === item.ibest) {
        let indices = this.query.length > 0 ? atom.ui.fuzzyMatcher.match(item.texts[item.ibest], this.query, { recordMatchIndexes: true }).matchIndexes : []
