...
export default class ProjectList {
  ...
  constructor(S) {
    ...
    this.objectArrayFilterer = new zadeh.ObjectArrayFilterer()
    this.query = ''
    ...
    if (this.windowTitle) { this.patchWindowTitle() ; this.update() }
  }
  ...
  cache() {
    return new Promise((resolve, reject) => {
      ...
      for (let item of this.items) {
        if (item.scan) { for (let dirPath of item.paths) {
          tasks.push(this.scanDir(item, dirPath))
        }}
      }
  ...
  scanDir(masterItem, dirPath) {
    return new Promise((resolve, reject) => {
      ...
      task.on('project-list:scan', (data) => {
