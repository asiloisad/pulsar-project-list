...
export default class ProjectList {
  ...
  constructor(S) {
    ...
    this.objectArrayFilterer = new zadeh.ObjectArrayFilterer() ; this.query = ''
    ...
    this.disposables.add(
      ...
      new File(this.getPath()).onDidChange(() => { this.restart = true }),
    )
  }
  ...
  updateInfoMessage() {
    this.objectArrayFilterer.setCandidates(this.items, 'text')
  ...
  cache() {
    return new Promise((resolve, reject) => {
      ...
      if (!fs.existsSync(naviPath)) { this.items = [] ; return reject() }
      let result = CSON.parseFile(naviPath, {})
      if ( result instanceof Error ) {
        console.error(result.stack)
        return reject()
      }
      for (let item of result) { this.prepareItem(item) ; this.items.push(item) }
      this.updateLoadingMessage()
      let tasks = []
      for (let item of this.items) {
        if (item.scan) { for (let dirPath of item.paths) { tasks.push(this.scanDir(item, dirPath)) }}
      }
  ...
  scanDir(masterItem, dirPath) {
    return new Promise((resolve, reject) => {
      ...
      task.on('project-list:entries', (data) => {
        for (let entry of data.entries) {
          let item = { title: path.basename(entry), tags: masterItem.tags, paths: [entry]}
  ...
  filter(items) {
    ...
    return this.objectArrayFilterer.filter(this.query, { maxResults: this.getMaxResults() })
  }
  ...
  elementForItem(item, options) {
    ...
    let total = 0
    let indices = this.query.length>0 ? zadeh.match(item.text, this.query) : []
    if (item.tags) {
