const { Disposable, CompositeDisposable, File } = require("atom");
const {
  SelectListView,
  highlightMatches,
...
module.exports = class ProjectList {
  ...
  elementForItem(item, { matchIndices }) {
    ...
    // Render tags first (visual order) - offset: title.length + space + accumulated
    let tagOffset = item.title.length + 1;
    if (item.tags) {
      for (let tag of item.tags) {
        let et = document.createElement("span");
        et.classList.add("tag");
        tagOffset += 1; // for #
        et.appendChild(highlightMatches(tag, indices.map((x) => x - tagOffset)));
        tagOffset += tag.length + 1; // tag + space
        e1.appendChild(et);
      }
    }

    // Parse and render [tag] patterns from title (if enabled) - offset: 0
    if (this.parseTitleTagsEnabled) {
      ...
      for (let part of titleParts) {
        if (part.isTag) {
          ...
          let text = "[" + part.text + "]";
          et.appendChild(highlightMatches(text, indices.map((x) => x - titleOffset)));
          titleOffset += text.length;
          e1.appendChild(et);
        } else {
          e1.appendChild(
            highlightMatches(part.text, indices.map((x) => x - titleOffset))
          );
          titleOffset += part.text.length;
        }
      }
    } else {
      // Render title as-is - offset: 0
      e1.appendChild(highlightMatches(item.title, indices));
    }
  ...
  scanDir(dirPath, tags, scanList) {
    return new Promise((resolve, reject) => {
      ...
      const workerPath = path.join(__dirname, "project-scan.js");
      const child = fork(workerPath, [], {
        cwd: __dirname,
        silent: true,
      });
      child.send({ dirPath, scanList });
  ...
  prepareItem(item) {
    // Format: "Title #tag1 #tag2" - title first for better scoring
    ...
    item.paths = item.paths.map((ppath) => {
      return ppath.replace(/[\\/]+$/, '').split(/[\\/]/g).join(path.sep) + path.sep;
    });
    return item;
  }
