...
export default class ProjectList {
  ...
  constructor(S) {
    ...
    this.slist = new SelectListView({
      ...
      filterQuery: (query) => { return Diacritics.clean(query) }
    })
  ...
  cache() {
    return new Promise((resolve) => {
      ...
      for (let item of this.items) { this.prepareItem(item) }
      this.updateLoadingMessage()
      let promises = []
  ...
  elementForItem(item, options) {
    ...
    let total = 0 ; let query = this.slist.getQuery()
    let indices = zadeh.match(item.text, query)
    if (item.tags) {
