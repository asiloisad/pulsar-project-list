...
module.exports = class ProjectList {
  ...
  elementForItem(item, { matchIndices }) {
    ...
    for (let dirPath of item.paths) {
      ...
      const icon = item.devMode ? "icon-beaker" : item.safeMode ? "icon-shield" : item.icon ? item.icon : "icon-file-directory";
  ...
  prepareItem(item) {
    ...
    item.text = item.title + (item.tags ? " " + item.tags.map((x) => `#${x}`).join(" ") : "");
