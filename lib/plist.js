...
module.exports = class PList {
  ...
  constructor() {
    ...
    this.disposables.add(
      atom.config.onDidChange('project-list.windowTitle', (value) => {
  ...
  async buildCache() {
    ...
    let configData = CSON.parseFile(this.configFile.getPath(), {})
    if (configData instanceof Error) {
      console.error(configData.stack)
      throw new Error('Cannot parse "projects.cson" file')
    }
    this.items = []
